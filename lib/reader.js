...
function Reader(database) {
  ...
  this.fileStream = fs.readFileSync(database);

  this.metadata = new Metadata(this.fileStream);
  this.decoder = new Decoder(this.fileStream, this.metadata.searchTreeSize + DATA_SECTION_SEPARATOR_SIZE);
...
var readNodeRight24 = function(offset) {
  return utils.concat3(
    this.fileStream[offset + 3],
    this.fileStream[offset + 4],
    this.fileStream[offset + 5]
  );
}

var readNodeLeft24 = function(offset) {
  return utils.concat3(
    this.fileStream[offset],
    this.fileStream[offset + 1],
    this.fileStream[offset + 2]
  );
}
...
var readNodeRight28 = function(offset) {
  return utils.concat4(
    this.fileStream[offset + 3] & 0x0f,
    this.fileStream[offset + 4],
    this.fileStream[offset + 5],
    this.fileStream[offset + 6]
  );
}

var readNodeLeft32 = function(offset) {
  return this.fileStream.readUInt32BE(offset, true);
}

var readNodeRight32 = function(offset) {
  return this.fileStream.readUInt32BE(offset + 4, true);
}
