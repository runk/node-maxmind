...
function Reader(database) {
  ...
  var metadataDecoder = new Decoder(this.fileHandle, 0);
  var metadataArray = metadataDecoder.decode(
    this.findMetadataStart(this.fileHandle)
  );
  this.metadata = new Metadata(metadataArray.value);
  this.decoder = new Decoder(this.fileHandle, this.metadata.searchTreeSize + DATA_SECTION_SEPARATOR_SIZE);
