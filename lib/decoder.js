...
Decoder.prototype.decodeSync = function(offset) {
  var tmp,
...
Decoder.prototype.decodeByTypeSync = function(type, offset, size) {
  ...
  switch (type) {
    case 'utf8_string':
      // return [decodeString(this.fileStream, offset, size), newOffset];
...
Decoder.prototype.decodeArraySync = function(size, offset) {
  var tmp,
    i = 0,
    array = []
    ;
...
Decoder.prototype.decodeMapSync = function(size, offset) {
  var tmp, key,
    map = {};
...
var decodeString = function(stream, offset, size) {
  return stream.utf8Slice(offset, offset + size);
};
