...
Decoder.prototype.decode = function(offset) {
  ...
  if (type === 'extended') {
    ...
    if (tmp < 8) {
      throw new Error('Maxmind: Invalid Extended Type at offset ' + offset);
    }
  ...
  return this.decodeByType(type, tmp.offset, tmp.value);
};
...
Decoder.prototype.decodeByType = function(type, offset, size) {
  ...
  switch (type) {
    ...
    case 'bytes':
      ...
      return cursor(bytes, newOffset);
    case 'uint16':
      return cursor(this.decodeUint16(offset, size), newOffset);
    case 'int32':
      return cursor(this.decodeInt32(offset, size), newOffset);
    case 'uint64':
      var bytes = this.read(offset, size);
      return cursor(this.decodeUint64(bytes), newOffset);
    case 'uint128':
      var bytes = this.read(offset, size);
      return cursor(this.decodeUint128(bytes), newOffset);
...
Decoder.prototype.read = function(offset, numberOfBytes) {
  var buf;
  if (numberOfBytes === 0) {
    return new Buffer(0);
  }
...
Decoder.prototype.decodeUint32 = function(offset, size) {
  ...
  if (size == 1) return this.fileStream[offset];
  if (size == 2) return this.fileStream.readUInt16BE(offset, true);
  if (size == 3) return utils.concat3(this.fileStream[offset], this.fileStream[offset + 1], this.fileStream[offset + 2]);
...
Decoder.prototype.decodeUniInt = function(bytes) {
  ...
  if (l == 2) return utils.concat2(bytes[0], bytes[1]);
  if (l == 3) return utils.concat3(bytes[0], bytes[1], bytes[2]);
  if (l == 4) return utils.concat4(bytes[0], bytes[1], bytes[2], bytes[3]);
}


Decoder.prototype.decodeUint64 = function(bytes) {
  return this.decodeBigUint(bytes, 8);
};


Decoder.prototype.decodeUint128 = function(bytes) {
  return this.decodeBigUint(bytes, 16);
};


Decoder.prototype.decodeBigUint = function(bytes, size) {
  var integer = 0;
