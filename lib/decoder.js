...
function Decoder(db, baseOffset, opts) {
  ...
  if (opts.locales) {
    assert(Array.isArray(opts.locales), 'opts.locales should be an array');
    assert(opts.locales.every(function(key) { return locales[key]; }), 'opts.locales has unknown values');

    var blacklist = Object.assign({}, locales);
    opts.locales.forEach(function(key) { delete blacklist[key]; });
    this.localesBlacklist = blacklist;
  } else {
    this.localesBlacklist = null;
  }

  console.log('>>>', this.localesBlacklist, opts, new Error().stack)
}


Decoder.prototype.decode = function(offset, decodeSizeOnly) {
  var tmp,
  ...
  if (decodeSizeOnly) {
    // TODO: consider to move size calc to a separate fn which
    // can be reused here and in `decodeByType`.
    return cursor(null, size.offset + size.value);
  }
...
Decoder.prototype.decodeFast = function(offset, decodeSizeOnly) {
  var cached = this.cache.get(offset);
  if (cached) {
    return cached;
  }

  var result = this.decode(offset, decodeSizeOnly);
...
Decoder.prototype.decodeMap = function(size, offset) {
  ...
  for (var i = 0; i < size; i++) {
    ...
    // Main point in doing whitelisting is to avoid expensive
    // string read operation. It increases decoding speed by ~50%
    // on average.
    //
    // We should potentially check for the full path of the node,
    // instead of relying on the single prop name only. It could
    // be error prone potentially, despite positive tests.
    if (this.localesBlacklist && this.localesBlacklist[key]) {
      tmp = this.decode(tmp.offset, true);
      offset = tmp.offset;
    } else {
