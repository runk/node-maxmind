...
function bsearch(arr, find) {
    var low = 0, high = arr.length - 1, i;

    while (low <= high) {
        i = Math.floor((low + high) / 2);
        if (arr[i][0] > find) { high = i - 1; continue; }
        if (arr[i][0] < find) { low  = i + 1; continue; }
        break;
    }

    return arr[i][0] == find ? arr[i] : -1;
}
...
function _makeCache() {
    ...
    function _buildIndex(depth, offset) {
        ...
        try {
            ...
            console.log('Error ---->', dbbuffer.length, 2 * recordLength * offset)
        }
