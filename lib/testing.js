...
// var source = fs.readFileSync(__dirname + '/../test/dbs/GeoLiteCity.dat');
var source = fs.readFileSync(__dirname + '/../test/dbs/GeoIP.dat');
// var source = fs.readFileSync('./GeoIP.dat');

var file = new Buff(source);
...
function seekCountry(ipAddress) {

    var buf = new Buffer(2 * MAX_RECORD_LENGTH);
    // console.log('buf len', buf.length);
    // console.log('dseg', databaseSegments);
    // console.log('rlen', recordLength);
    ...
    for (var depth = 31; depth >= 0; depth--) {
        ...
        if ((dboptions & GEOIP_MEMORY_CACHE) == 1) {
            ...
            // }
            // console.log(2 * recordLength * offset, (2 * recordLength * offset) + (2 * MAX_RECORD_LENGTH));
    ...
    console.log("Error seeking country while seeking " + ipAddress);
    return 0;
};
    ...
    try {
        ...
        if ((dboptions & GEOIP_MEMORY_CACHE) == 1) {
            ...
            dbbuffer.copy(
                ...
                0,
                record_pointer,
                record_pointer + Math.min(dbbuffer.length - record_pointer, FULL_RECORD_LENGTH)
            );
        ...
        // var time = process.hrtime();
        // get region
        while (record_buf[record_buf_offset + str_length] != 0x00)
            str_length++;
        ...
        // var diff = process.hrtime(time);
        // perf.postcode += diff[0] * 1e9 + diff[1];

        // get latitude
        // for (j = 0; j < 3; j++)
        //     latitude += record_buf.readUInt8(record_buf_offset + j) << (j * 8);
        ...
        // get longitude
        // for (j = 0; j < 3; j++)
        //     longitude += record_buf.readUInt8(record_buf_offset + j) << (j * 8);
...
console.log('---------------------------------------------')
console.log(databaseInfo.toString());
console.log(databaseInfo.getDate());
console.log('---------------------------------------------')

// console.log(getLocation('109.60.171.33'))

// var s = new Date().getTime();
// for (var i = 0; i < 100000; i++) {
//     getLocation('109.60.171.33');
// };
// console.log(new Date().getTime() - s);
// console.log(perf)

console.log(seekCountry(ip2long('109.60.171.33')))
console.log(getCountry('109.60.171.33').getCode())
